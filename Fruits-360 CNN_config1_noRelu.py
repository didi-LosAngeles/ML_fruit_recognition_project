...
# the expected input shape for the trained models; since the images in the Fruit-360 are 100 x 100 RGB images, this is the required input shape
...
# link for folder:  C:\Users\dcozi\Visual Studio Code\ML_fruit recognition\source work\Fruit-Images-Dataset-master
# modified link for python:  C:/Users/dcozi/Visual Studio Code/ML_fruit recognition/source work/Fruit-Images-Dataset-master
...
def plot_model_history(model_history, out_path=""):
    ...
    axs[0].set_xticks(np.arange(
        1, len(model_history.history['acc']) + 1), len(model_history.history['acc']))
    ...
    axs[1].set_xticks(np.arange(
        1, len(model_history.history['loss']) + 1), len(model_history.history['loss']))
...
def plot_confusion_matrix(y_true, y_pred, classes, out_path=""):
    ...
    if out_path:
        # as in the plot_model_history, the matrix is saved in a file called "model_name_confusion_matrix.png"
...
def network(input_shape, num_classes):
    ...
    x = Flatten()(x)
    x = Dense(1024, activation='elu', name='fcl1')(x)
    x = Dropout(0.2)(x)
    x = Dense(256, activation='elu', name='fcl2')(x)
    x = Dropout(0.2)(x)
...
def train_and_evaluate_model(model, name="", epochs=25, batch_size=50, verbose=verbose, useCkpt=False):
    ...
    plot_confusion_matrix(y_true, y_pred.argmax(axis=-1),
                          labels, out_path=model_out_dir)
